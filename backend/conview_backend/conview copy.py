...
def render_img(atlas_img: nib.Nifti1Image, bg_img:nib.Nifti1Image, four_d_img: nib.Nifti1Image, idx_4d: int, 
    ...
    if infos_r.vrange in [(None, None), (-2, 2)]:
        data4d = four_d_img.get_fdata()  # (X, Y, Z, T)
        vmin4d, vmax4d = compute_global_vrange(data4d, infos_r.threshold)
        infos_r.vrange = (vmin4d, vmax4d)
        print(f"[CMAP] 4D global vrange set for connectivity: vmin={vmin4d:.4f}, vmax={vmax4d:.4f}")
...
def make_scale(normalizer, color_map):
    return pp.LinLayout("v")(
            pp.LLItem(2),
            pp.LLItem(2)(pp.Padding(0.1, 0.1, 0, 0)(make_color_map_surface(color_map))),
            pp.LLItem(1),
            pp.LLItem(4)(
